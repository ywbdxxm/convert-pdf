...
`tables/*.csv` and `tables/*.html` are verification layers. When a value matters, use table sidecars and the original PDF page to cross-check.
...
- OCR/GPU/image/cache settings
- output paths
- table count and table records
- alert count and alert records
- conversion signature
- window status when window cache is enabled

Start here when an AI assistant is given a processed manual directory.
...
```text
Table sidecars: [HTML](tables/table_XXXX.html) | [CSV](tables/table_XXXX.csv) | `doc_id:table:XXXX`
```
...
Optional window cache.

Only created/used when `--enable-window-cache` is provided. Each cached window includes:
...
If conversion settings change, the cache is invalidated.
...
- no OCR
- images referenced as sidecars
- table sidecars exported
- no window cache
...
### 2. Use `sections.jsonl` to navigate
...
### 3. Use `chunks.jsonl` for retrieval
...
### 6. Use `document.html` or `artifacts/` for visual content
